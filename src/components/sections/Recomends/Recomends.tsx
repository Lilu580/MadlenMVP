...
export const Products = (): JSX.Element => {
  return (
    <section className="flex flex-col items-start gap-[50px] relative mx-auto mt-28">
      <div className="flex items-center justify-between w-full">
        <h2 className="text-2xl text-[#0A0B0C] font-[garamond]">НОВИНКИ</h2>
        <Button content="Переглянути все" />
      </div>

      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {products.map((product, index) => (
          <RecomendationCart key={`${product.name}-${index}`} product={product} />
        ))}
      </div>
    </section>
  );
